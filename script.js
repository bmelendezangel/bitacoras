...
function updateMantenimientosVentasTable() {
  ...
  mantenimientosVentas.forEach(item => {
    const puntosTipoA = item.tipoMantenimiento === 'tipoA' ? item.numeroVentas * 3 : 0;
    const puntosTipoB = item.tipoMantenimiento === 'tipoB' ? item.numeroVentas * 7 : 0;
    ...
    row.insertCell().textContent = item.tipoMantenimiento;
...
function updateKpiTable() {
  ...
  let sumaTipoAyB = 0;
  mantenimientosVentas.forEach(item => {
    if (item.tipoMantenimiento === 'tipoA') {
      sumaTipoAyB += item.numeroVentas;
    } else if (item.tipoMantenimiento === 'tipoB') {
      sumaTipoAyB += item.numeroVentas;
    }
  });
  const diasRealizados = contarDiasConMantenimiento();
